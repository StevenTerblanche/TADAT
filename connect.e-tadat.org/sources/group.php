...
function PageMain() {
	...
	if($_GET['r'] == 'edit' && $feed->group_member_data['permissions'] == 2) {
		...
		$TMPL['edit_url'] = '&name='.$feed->group_data['name'].'&r=edit';
		$TMPL['delete_url'] = '&name='.$feed->group_data['name'].'&r=delete';
		...
		if(!empty($_POST)) {
			...
			if($message[0]) {
				...
				header("Location: ".$CONF['url']."/index.php?a=group&name=".$message[1]);
			}
		}
	}
