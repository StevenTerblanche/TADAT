	...
	if($user !== false) {
		...
		*/
		$links = array(	array('profile&u='.$user['username'], realName($user['username'], $user['first_name'], $user['last_name']), 1, 0),
						...
						array('settings&b=avatar', $LNG['title_settings'], 1, 0),
...
//echo ''.$this->password;
	...
	function logOut() {
		...
		header("Location: https://portal.e-tadat.org");
		die();
	}
}
...
class logInAdmin {
	...
	function in() {
		...
		if($this->queryLogIn() == 1) {
			...
			$_SESSION['passwordAdmin'] = $this->password;
...
///				echo $getOriginal;
	...
	function getGroup($start, $group, $from = null) {
		...
		if($this->group_data['name']) {
			...
			return $this->showError('group_not_exist');
		}
	}
...
/*
			$query = sprintf("
				SELECT `idu`, `username`, `email`, `first_name`, `last_name`, `country`, `location`, 
				`address`, `school`, 
				`work`, 
				`website`, `bio`, `date`, 
				`facebook`, `twitter`, `gplus`, 
				`image`, `private`, `suspended`, `privacy`, 
				`born`, `cover`, `verified`, 
				`gender`, 
				`interests`, `email_new_friend` 
				FROM `users` WHERE `idu` = '%s'", $this->db->real_escape_string($id));


*/
			$query = sprintf("
				SELECT `idu`, `fkidUserId`, `username`, `email`, `first_name`, `last_name`, 
				`date`, 
				`image`, `private`, `suspended`, `privacy`, 
				`cover`, `verified`, 
				`email_new_friend` 
...
/*
			$query = sprintf("
				...
				`gender`, `interests`, 
				`email_new_friend` 
				FROM `users` WHERE `username` = '%s'", $this->db->real_escape_string($username));
*/
			$query = sprintf("
				SELECT `idu`, `fkidUserId`, `username`, `email`, `first_name`, `last_name`, 
				`date`, 
		...
		// Run the query
		$result = $this->db->query($query);
		$arr_profile = $result->fetch_assoc();

		if($id){
			$id_query = sprintf("
				SELECT `fkidUserId`	FROM `users` WHERE `idu` = '%s'", $this->db->real_escape_string($id));
		}else{
			$id_query = sprintf("
				SELECT `fkidUserId` FROM `users` WHERE `username` = '%s'", $this->db->real_escape_string($username));
		}

		// GET THE PORTAL USER ID
		$id_result = $this->db->query($id_query);
		$arr_id_result = $id_result->fetch_assoc();
		$fkidUserId = $arr_id_result['fkidUserId'];
...
*/
		...
		if($fkidUserId){
			$db_portal = new mysqli('dedi972.jnb1.host-h.net', 'portazyyvj_1', 'd2X7432AqqT6pmaM', 'tadat_portal');
			if ($db_portal->connect_errno) {
				echo "Failed to connect to MySQL: (" . $db->connect_errno . ") " . $db->connect_error;
			}
			$db_portal->set_charset("utf8");
			
			// GET THE RETREIVED USERS INTEREST IDS
			$query_portal_interests_ids = sprintf("SELECT UserProfile.fkid_interests FROM `UserProfile` WHERE `fkidUserId` = " . $fkidUserId);
			$result_portal_interests_ids = $db_portal->query($query_portal_interests_ids);
			$arr_result_portal_interests_ids = $result_portal_interests_ids->fetch_assoc();
			$str_query_portal_interests_ids = implode(',', $arr_result_portal_interests_ids);

			// GET THE USERS INTEREST FROM IDS
			$query_portal_interests_from_id = sprintf("SELECT `interests` FROM `UserInterests` WHERE `id` IN (".$str_query_portal_interests_ids).")";
			$result_portal_interests = $db_portal->query($query_portal_interests_from_id);
			$RetreivedInterests = false;
			$strRetreivedInterests = '';
			if($result_portal_interests){
				while ($row = $result_portal_interests->fetch_array()) {
					$strRetreivedInterests .= '<li>'.$row['interests'].'</li>';
					$RetreivedInterests = true;				
				}
			}
			if($RetreivedInterests){
				$arr_profile['interests'] = '<ul style="margin-left:5px !important; padding-left:5px !important;">'.$strRetreivedInterests.'</ul>';
			}else{
				$arr_profile['interests'] ='No Interests Specified';
			}

			// GET THE USERS OCCUPATION ID
			$query_portal_occupation = sprintf("SELECT UserProfile.fkid_occupation, UserOccupation.occupation FROM `UserProfile` JOIN `UserOccupation` ON  UserOccupation.id = UserProfile.fkid_occupation	WHERE `fkidUserId` = " . $fkidUserId);
			$result_portal_occupation = $db_portal->query($query_portal_occupation);
			$RetreivedOccupation = false;
			$strRetreivedOccupation = '';
			if($result_portal_occupation){
				while ($row = $result_portal_occupation->fetch_array()) {
					$strRetreivedOccupation .= ucwords($row['occupation']);
					$RetreivedOccupation = true;				
				}
			}
			if($RetreivedOccupation){
				$arr_profile['occupation'] = $strRetreivedOccupation;
			}else{
				$arr_profile['occupation'] ='No Occupation Specified';
			}

			// GET THE USERS LOCATION and Company
			$query_portal_location = sprintf("SELECT UserProfile.current_organization, UserProfile.current_business_address_city, Countries.country 
			FROM `UserProfile` 
			JOIN `Countries` ON  Countries.id = UserProfile.current_fkid_country_business
			WHERE `fkidUserId` = " . $fkidUserId);
			$result_portal_location = $db_portal->query($query_portal_location);
			$RetreivedLocation = false;
			$strRetreivedCountry = '';
			$strRetreivedCity = '';
			$strRetreivedCompany = '';			
			if($result_portal_location){
				while ($row = $result_portal_location->fetch_array()) {
					$strRetreivedCountry .= $row['country'];
					$strRetreivedCity .= $row['current_business_address_city'];					
					$strRetreivedCompany .= $row['current_organization'];
					$RetreivedLocation = true;				
				}
			}
			if($RetreivedLocation){
				$arr_profile['country'] = $strRetreivedCountry;				
				$arr_profile['location'] = $strRetreivedCity;								
				$arr_profile['work'] = $strRetreivedCompany;

			}else{
				$arr_profile['country'] = 'None Specified';
				$arr_profile['location'] = 'None Specified';
				$arr_profile['work'] = 'None Specified';
			}
		...
		<div class="message-container">
			<div class="message-content">
				<div class="message-inner">
					...
					'.(($contactSection) ? '<div><strong>'.$LNG['contact_information'].'</strong></div>
					'.(($work)		? '<div class="about-row"><div class="about-text">'.$LNG['works_at'].'</div><div class="about-text">'.$work.'</div></div>' : '').'
					'.(($occupation)	? '<div class="about-row"><div class="about-text">'.$LNG['occupation'].'</div><div class="about-text">'.$occupation.'</div></div>' : '').'
					'.(($location)	? '<div class="about-row"><div class="about-text">'.$LNG['location'].'</div><div class="about-text">'.$location.'</div></div>' : '').'
					'.(($interests) ? '<div class="about-row"><div class="about-text">'.$LNG['interests'].'</div><div class="about-text">'.$interests.'</div></div>' : '').'<br>' : '').'
...
/*		$widget =  '<div class="sidebar-container widget-welcome">
						<div class="sidebar-content">
							...
							<div class="sidebar-inner">
								...
								<div class="sidebar-avatar-desc">
									<a href="'.$this->url.'/index.php?a=profile&u='.$username.'" rel="loadpage">'.((!empty($name) ? $name : $username)).'</a>
									<div class="sidebar-avatar-edit">444<a href="'.$this->url.'/index.php?a=settings" rel="loadpage">'.$LNG['admin_ttl_edit_profile'].'</a></div>
								</div>
							</div>
						</div>
					</div>';
	*/				
	...
	function sidebarProfileInfo($profile) {
		...
		$rows = array(
			...
			$LNG['location']			=> array('location', $country),
		...
		$info = '<div class="sidebar-container widget-about"><div class="sidebar-content"><div class="sidebar-header"><a href="'.$this->url.'/index.php?a=profile&u='.$this->profile.'&r=about" rel="loadpage">'.$LNG['profile_about'].'</a></div>';
//		$info = '<div class="sidebar-container widget-about"><div class="sidebar-content"><div class="sidebar-header"><a href="'.$this->url.'/index.php?a=profile&u='.$this->profile.'&r=about" rel="loadpage">'.$LNG['profile_about'].'</a>'.(($this->profile == $this->username) ? ' <span class="sidebar-header-link"><a href="'.$this->url.'/index.php?a=settings" rel="loadpage">'.$LNG['admin_ttl_edit'].'</a></span>' : '').'</div>';		
	...
	function parseMessage($message) {
		...
		$parsedMessage = preg_replace(array('/(^|[^a-z0-9_])@([a-z0-9_]+)/i', '/(^|[^a-z0-9_])!([a-z0-9_]+)/i', '/(^|[^a-z0-9_])#(\w+)/u'), array('$1<a href="'.$this->url.'/index.php?a=profile&u=$2" rel="loadpage">@$2</a>', '$1<a href="'.$this->url.'/index.php?a=group&name=$2" rel="loadpage">!$2</a>', '$1<a href="'.$this->url.'/index.php?a=search&tag=$2" rel="loadpage">#$2</a>'), $parseUrl);
	...
	function sidebarPlaces($id) {
		...
		if(!empty($rows)) {
			...
			foreach($rows as $row) {
				...
				$output .= '<a href="'.$this->url.'/index.php?a=post&m='.$row['id'].'" rel="loadpage"><div class="sidebar-subscriptions"><div class="sidebar-title-container"><div class="sidebar-places-name">'.$row['value'].'</div></div><img src="https://maps.googleapis.com/maps/api/staticmap?center='.$row['value'].'&zoom=13&size=150x150&maptype=roadmap&sensor=false&scale=2&visual_refresh=true" referrerpolicy="no-referrer"></div></a>';
