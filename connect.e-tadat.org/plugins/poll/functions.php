...
class Poll extends Feed {
	function pollVote($id, $value) {
		...
		$query = $this->db->query(sprintf("SELECT `id`,`idu`,`username`,`private`,`public`,`message`,`type`,`value` FROM `messages`, `users` WHERE `messages`.`value` LIKE '%s' AND `messages`.`uid` = `users`.`idu`", $this->db->real_escape_string('poll:{"id":'.$id.',%')));
