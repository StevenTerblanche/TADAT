...
class Scores extends Core_Controller {
	...
	function poas(){
		...
		$this->panel_title = strtoupper(lang('performance_outcome_areas') . lang('_status'));	
 		$data = $this->includes;
		$content_data['get_record_all_poas'] = _get_record_all_poa('1','db_b');
	...
	function pis(){
		...
		$data = $this->includes;
		$content_data['get_record_all_pis'] = _get_all_fields_by_id_array('db_b','PerformanceIndicators','fkidPoa',$this->session->userdata('pi_id'));
	...
	function dimensions(){
		...
		$data = $this->includes;
		$content_data['get_record_table'] = _get_score_by_id_all('db_b','PerformanceIndicators','id',$this->session->userdata('pi_id'));
