...
class Questions extends Core_Controller {
	...
	function dimensions(){
		...
		$this->content_data['measurement_dimensions'] = _get_record_by_fkid_md($this->session->userdata('indicator_id') ,'','db_b');
		...
		if($this->id !== 'pni' && $this->action !== 'create'){
			$this->content_data['get_record_by_id'] = _get_all_fields_by_id('db_b', $this->tbl, 'id', $this->id);
		...
		if($this->action === 'save'){
			...
			$updateId = $this->crud_model->crud($this->action, $this->tbl, $this->form_data, $this->id, $this->fkID, 'db_b');
			$this->content_data['get_record_by_id'] = _get_all_fields_by_id('db_b', $this->tbl, 'id', $updateId);							
...
//					$this->form_validation->set_rules($fields, form_label($fields, $fields), 'required|trim|decimal|greater_than[0]');
				...
				if($this->action === 'score'){
					...
					if(_score_indicator_tables($this->tbl)){
						...
						if($this->id && $this->id !== 'pni'){
							...
							$this->action = 'create';
							$this->form_data['status'] = 0;
							$this->id = $this->crud_model->crud($this->action, $this->tbl, $this->form_data, $this->id, $this->fkID, 'db_b');
						}
	...
	function view_db(){
		$this->content_data['id'] = $this->encrypt->decode(base64_decode($this->input->get('id')));
		//$this->content_data['id'] = $this->input->get('id');
		$this->data['content'] = $this->load->view('questionnaire/poa/view', $this->content_data, true);
        $this->load->view('questionnaire/poa/view', $this->data);
	...
	function score(){
		...
		$this->content_data['uri'] = 'https://e-tadat.org/questions/report/'.base64_encode($this->encrypt->encode($this->id)).'/'.base64_encode($this->encrypt->encode($this->session->userdata('mission_id'))).'/'.base64_encode($this->encrypt->encode($this->session->userdata('poa_id'))).'/'.base64_encode($this->encrypt->encode($this->session->userdata('indicator_id')));	
		$this->content_data['get_record_table'] = _get_score_by_id_all('db_b','PerformanceIndicators','id',$this->session->userdata('indicator_id'));
...
//		$this->form_data['status'] = 6;
		...
		$this->crud_model->crud('update', $this->session->userdata('tbl'), $this->form_data, $this->id, $this->fkID, 'db_b');
		...
		$this->content_data['uri'] = 'https://e-tadat.org/questions/submit/'.base64_encode($this->encrypt->encode($this->id)).'/'.$this->session->userdata('mission_id').'/'.$this->session->userdata('poa_id');	
		$this->content_data['get_record_table'] = _get_score_by_id_all('db_b','PerformanceIndicators','id',$this->session->userdata('indicator_id'));
