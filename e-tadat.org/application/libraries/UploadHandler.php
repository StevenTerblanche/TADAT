...
class UploadHandler{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null){
        ...
        $this->options = array(
            ...
            'user_dirs' => true,
			'database' => 'etadat_questions',
			'host' => 'dedi972.jnb1.host-h.net',
			'username' => 'etadarwskd_2',
			'password' => '3u267Q32WpyXn8wY',
            'mkdir_mode' => 0755,
            'param_name' => 'files',
            'delete_type' => 'DELETE',
...
//            'accept_file_types' => '/.+$/i',
			'accept_file_types' => '/\.(gif|jpe?g|pdf|doc|docx|rtf|xls|xlsx|ppt|pptx|pps|ppsx)$/i',
            'max_file_size' => null,
            'min_file_size' => 1,
...
/*				
			...
			$file->upload_to_db = $this->add_img($file->title, $file->description, $name, $fileSize, $file->type, $short_path, $thumb_path, $data, $dateCreated, $file->fkidUserCreated, $file->fkidIndicator, $file->fkidDimension, $this->options['fkidMission'], $this->options['uploadType']);	
			unlink($file_path);				
		}
		return $file;
	}
	...
	function add_img($title, $description, $name, $size, $type, $path, $thumb_path, $data, $dateCreated, $fkidUserCreated, $fkidIndicator, $fkidDimension, $fkidMission, $uploadType){
		
		$add_to_db = $this->query("INSERT INTO EvidentiaryDocumentation (
			documentTitle ,documentDescription, documentName, documentSize, documentType, documentPath, thumbPath, documentData, dateCreated, fkidUserCreated, fkidIndicator, fkidDimension, fkidMission, uploadType 
		) VALUES(
			'{$title}',	'{$description}', '{$name}', '{$size}', '{$type}', '{$path}', '{$thumb_path}', '{$data}', '{$dateCreated}', '{$fkidUserCreated}', '{$fkidIndicator}','{$fkidDimension}', '{$fkidMission}', '{$uploadType}'
		)") or die(mysql_error());  
