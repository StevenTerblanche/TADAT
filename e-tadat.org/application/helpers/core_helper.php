...
function _get_login_attempts($id){
	if (is_null($id))return false;
	get_instance()->db->select('Users.loginAttempts');
	get_instance()->db->from('Users');
	get_instance()->db->where('id', $id);
	$query = get_instance()->db->get();
	$ret = $query->row();
	return $ret->loginAttempts;
}


function _get_account_status($id){
	if (is_null($id))return false;
	get_instance()->db->select('Users.status');
	get_instance()->db->from('Users');
	get_instance()->db->where('id', $id);
	$query = get_instance()->db->get();
	$ret = $query->row();
...
function _get_account_level($id){
	if (is_null($id))return false;
	get_instance()->db->select('Users.fkidUserRole');
	get_instance()->db->from('Users');
	get_instance()->db->where('id', $id);
	$query = get_instance()->db->get();
	$ret = $query->row();
	return $ret->fkidUserRole;
}
...
function _score_indicators($indicatorId,$score_number_dimension_1,$score_number_dimension_2,$score_number_dimension_3,$score_number_dimension_4,$score_number_dimension_5){
	if($indicatorId){
		...
		#	Get all Scoring Methods by Dimension for the specified Performance Indicator
			$scoring_method_array = _get_scoring_method_by_indicator('db_b', 'MeasurementDimensions', $indicatorId, array('fkidScoringMethod','fkidDimensionType'));
