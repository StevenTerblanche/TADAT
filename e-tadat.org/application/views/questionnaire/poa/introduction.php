		...
		<div class="row profile">
			<div class="col-md-12">
				...
				<?php
					...
					echo '<p>The '.$objGetMissionDetails->missionName. ', covering the assessment periods of ' . $objGetMissionDetails->missionAssessmentPeriod . ', was created on behalf of the <a href="https://e-tadat.org/authorities/profile/?id='.base64_encode($this->encrypt->encode($objGetMissionDetails->authorityId)).'" target="_blank">'.$objGetMissionDetails->authorityName.'</a> Tax Administration, on '.date("j F Y", strtotime($objGetMissionDetails->missionDateCreated)).', by '
					.'<a href="https://e-tadat.org/users/profile/?id='.base64_encode($this->encrypt->encode($objGetMissionDetails->secId)).'" target="_blank">'.$objGetMissionDetails->secUserTitle.' '.$objGetMissionDetails->secUserName.' '.$objGetMissionDetails->secUserSurname.'</a> ('.$objGetMissionDetails->secUserRole.').' 
					.'</p>'; 
					...
					echo '<p>The primary contact person at the Tax Administration is <a href="https://e-tadat.org/users/profile/?id='.base64_encode($this->encrypt->encode($objGetMissionDetails->racId)).'" target="_blank">'.$objGetMissionDetails->racUserTitle.' '.$objGetMissionDetails->racUserName.' '.$objGetMissionDetails->racUserSurname.'</a> ('.$objGetMissionDetails->racUserRole.').'; 
					if($objGetMissionDetails->raId){
						echo ' Secondary contact may also be made to <a href="https://e-tadat.org/users/profile/?id='.base64_encode($this->encrypt->encode($objGetMissionDetails->raId)).'" target="_blank">'.$objGetMissionDetails->raUserTitle.' '.$objGetMissionDetails->raUserName.' '.$objGetMissionDetails->raUserSurname.'</a> ('.$objGetMissionDetails->raUserRole.').'; 
					}
					echo '</p>';
				?>
				</div>
...
<?php
	...
	$arrPoaListing = _get_selected_records_poa(null,'db_b');
	$arrGetAllAssignmentsByMissionId = _get_record_all_assignments($arrMissions = array());
		foreach($arrPoaListing as $poa_index => $poa_value){
				$strStatusType = (_get_status_by_id_all('db_b', 'PerformanceIndicators', $missionId, $poa_value['id']));
				$strAssignedPoa = 0;
