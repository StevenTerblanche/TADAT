<?php defined('BASEPATH') OR exit('No direct script access allowed'); 
		...
		echo '--------------------- KEY -----------------------------<br>';
		echo '<pre>';
		print_r($getarrObject);
		echo '</pre>';
		echo '--------------------------------------------------<br>';

if(isset($getarrObject)){
?>
...
<?php
	foreach($getarrObject as $key => $key_values){
		if ($key === 'so'){
			foreach($key_values as $so => $so_values){
	?>
		...
		?>
										<table class="table table-bordered table-striped table-hover dt-responsive non-responsive mb10" cellspacing="0" width="100%">
											...
											<tbody>		
		<?php	
			foreach($key_values as $poa => $poa_values){
				$strStatusType = (_get_status_by_id_all('db_b', 'PerformanceIndicators', $so_values['mission_id'], $poa_values['id']));
				if($strStatusType == 1){
