<?php defined('BASEPATH') OR exit('No direct script access allowed'); 
	...
	if($arrDimensions){
		...
		foreach($arrDimensions as $key => $value){
			$cnt++;
		$methodDimension_1 = _get_record_by_id_score_method($arrDimensions[$key]['id'], $results[0]['score_number_dimension_1'], 'db_b', 'MeasurementDimensionsScoring', $fields = array('criteria'));	
			if($key == 0) {
				echo '<tr class="va">';
