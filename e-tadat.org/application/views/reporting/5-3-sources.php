				...
				<td>
					...
					<?php foreach($arrSources as $key => $value){
							if($value['fkidIndicator'] == 1){
								echo '<li><a href="'. base_url('data/view?id=').base64_encode($this->encrypt->encode($value['id'])) . '" target="_blank">'.$value['documentTitle'].'</a></li>';
							}
						}?>
					</ul>
				</td>
				...
				<td>
					...
					foreach($arrSources as $key => $value){
							if($value['fkidIndicator'] == 2){
								$strIndicator_2 .= '<li><a href="'. base_url('data/view?id=').base64_encode($this->encrypt->encode($value['id'])) . '" target="_blank">'.$value['documentTitle'].'</a></li>';
				...
				<td>
					...
					foreach($arrSources as $key => $value){
							if($value['fkidIndicator'] == 3){
								$strIndicator_3 .= '<li><a href="'. base_url('data/view?id=').base64_encode($this->encrypt->encode($value['id'])) . '" target="_blank">'.$value['documentTitle'].'</a></li>';
