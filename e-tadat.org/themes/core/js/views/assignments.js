$(document).ready(function() {
	...
	function set_mission(id, table, ignore){
		...
		$.ajax({
			async:false, data:data,	url:'https://e-tadat.org/ajax/populateMissionsForm/',
	...
	function set_user(authorityId, missionId, table, ignore, action){
		...
		$.ajax({
			async:false, data:data,	url:'https://e-tadat.org/ajax/populateUserPoaForm/',
	...
	function set_poa(authorityId, missionId, userId, table, ignore, action){
		...
		$.ajax({
			async:false, data:data,	url:'https://e-tadat.org/ajax/populatePoaForm/',
			success : function(data){
						$("#poa_data").html(''); 
						$("#poa_data").html(data); 
						$("#poa_container").show();
	...
	$('#back_button').click(function(){var url = 'https://e-tadat.org/assignments/list'; $(location).attr('href',url); return false;});	

	$('#create_assignment').click(function(){window.location.replace("https://e-tadat.org/assignments/create");});		
});