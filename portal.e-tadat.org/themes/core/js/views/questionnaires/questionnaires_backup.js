...
$(document).ready(function() {
   ...
   tinymce.init({
			menubar:false,
		    statusbar: false,
			selector: '.required-textarea-reason',
			height : 200,
			mode : "textareas",
   ...
   tinymce.init({
	...
	$('.required-radio').bind('change', function(){validateTadatRadio();window.validateTadatMemo();});

	window.validateTadatMemo = function(editorText){
		var flag = 0;
		var visibleLength = 0;			
		...
		required.each(function(index, element){
			...
			if(editorText && editorText != null){
				window.passedContent = editorText;
			}else{
				window.passedContent = $(this).val();
			}
			
//			window.assignedName = $(this).attr("id");
//			console.log("VAL: "+$(this).val());
//			console.log("VAL BY ID: "+window.tinyMCE.get('#id_dimension_6_17_1_2_c').getContent());
			
//			console.log("NAME: " + window.assignedName)			
//			window.passedContent = $(this).attr("value");
			console.log(window.passedContent);
			window.passedContentLength = window.passedContent.length;
			if (window.passedContentLength > 7){
				flag++;
			}
	...
	function ShowScoreButton(){
		...
		console.log('Score Memo:'+score_memo);
		console.log('Score Radio:'+score_radio);		
...
/*	$("#form-submit").submit(function () {
		...
		this_master.find('.check').each(function(){
			...
			if(checkbox_this.is(":checked") == false){ 
				checkbox_this.attr("style", "visibility: hidden")
