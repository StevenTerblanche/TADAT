...
class Pi extends Core_Controller {
	...
	*/
	function __construct(){
		...
		$this->add_external_js(array("/themes/core/js/jquery.dataTables.js", "/themes/core/js/dataTables.tableTools.js", "/themes/core/js/dataTables.bootstrap.js", "/themes/core/js/dataTables.responsive.js","/themes/core/js/bootstrap-datepicker.js","/themes/core/js/tinymce/tinymce.min.js", "/themes/core/js/views/pi.js"));
		...
		$this->tbl = 'PerformanceIndicators';
		...
		$this->columns = 12;
		$this->page_header = strtoupper(lang('manage_').lang('performance_indicators'));
...
//		$this->output->enable_profiler(TRUE);		
	...
	function read(){
		if(!$this->id){
			$this->panel_title = strtoupper(lang('performance_indicator') . lang('_listing'));
			$this->content_data['get_record_all'] = _get_record_all_pi('','cloud_questions');
			$this->data['content'] = $this->load->view('pi/list', $this->content_data, true);
		}
	...
	function modify(){
		...
		# This shows the 'create view' for a NEW record if no ID OR action 
			$this->content_data['get_dropdown_all_poa'] = _get_dropdown_all_poas();
	        $this->content_data['get_dropdown_all_languages'] = _get_dropdown_all_languages();
			$this->content_data['_get_dropdown_all_sms'] = _get_dropdown_all_sms();
		if(!$this->id && !$this->action){
			# Tell the crud function what to do.
			$this->action = 'create';
			# Populate view variables according to type
			$this->panel_title = lang('create_').lang('performance_indicator');
			$this->content_data['submit_button'] = lang('create_').lang('performance_indicator');
			...
			$this->data['content'] = $this->load->view('pi/create', $this->content_data, true);
		}
		...
		if($this->id && !$this->action){
			...
			$this->action = 'update';
			# Populate view variables according to type			
			$this->panel_title = lang('update_').lang('performance_indicator');
			$this->content_data['submit_button'] = lang('update_').lang('performance_indicator');
			...
			$this->content_data['get_record_by_id'] = _get_record_by_id_pi($this->id, '', 'cloud_questions');
	        $this->content_data['get_dropdown_all_languages'] = _get_dropdown_all_languages();			
		...
		if($this->action && ($this->action !=='disable' || $this->action !=='delete')){
			...
			$this->form_validation->set_rules('indicatorName', lang('performance_indicator').lang('_name'), 'required|trim');
