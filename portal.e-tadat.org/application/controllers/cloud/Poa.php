...
class Poa extends Core_Controller {
	...
	*/
	function __construct(){
		...
		$this->add_external_js(array("/themes/core/js/jquery.dataTables.js", "/themes/core/js/dataTables.tableTools.js", "/themes/core/js/dataTables.bootstrap.js", "/themes/core/js/dataTables.responsive.js","/themes/core/js/bootstrap-datepicker.js","/themes/core/js/tinymce/tinymce.min.js", "/themes/core/js/views/poa.js"));
		...
		$this->columns = 12;
		$this->page_header = strtoupper(lang('manage_').lang('performance_outcome_area'));
		$this->panel_icon = 'fa fa-sliders';
		$this->panel_title = strtoupper(lang('performance_outcome_area') . lang('_listing'));		
...
//		$this->output->enable_profiler(TRUE);		
	...
	function read(){
		if(!$this->id){
			$this->panel_title = strtoupper(lang('performance_outcome_area') . lang('_listing'));
			$this->content_data['get_record_all'] = _get_record_all_poa('','cloud_questions');
			$this->data['content'] = $this->load->view('poa/list', $this->content_data, true);
		}
	...
	function modify(){
		...
		if(!$this->id && !$this->action){
			...
			$this->panel_title = lang('create_').lang('performance_outcome_area');
			$this->content_data['submit_button'] = lang('create_').lang('performance_outcome_area');
			...
			$this->data['content'] = $this->load->view('poa/create', $this->content_data, true);
		}
		...
		if($this->id && !$this->action){
			...
			$this->action = 'update';
			# Populate view variables according to type			
			$this->panel_title = lang('update_').lang('performance_outcome_area');
			$this->content_data['submit_button'] = lang('update_').lang('performance_outcome_area');
			...
			$this->content_data['get_record_by_id'] = _get_record_by_id_poa($this->id, '1', 'cloud_questions');
	        $this->content_data['get_dropdown_all_languages'] = _get_dropdown_all_languages();			
		...
		if($this->action && ($this->action !=='disable' || $this->action !=='delete')){
			...
			$this->form_validation->set_rules('poa', lang('performance_outcome_area').lang('_name'), 'required|trim');
