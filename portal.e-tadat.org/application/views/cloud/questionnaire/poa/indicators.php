...
if($get_poa_by_id){
	foreach($get_poa_by_id as $question => $questions){
		...
		$arrIndicators = _get_indicators_by_poa_pi($questions['id'],'','cloud_questions');
...
//								echo 'POA Indicator ID: '.$strPoaIndicatorNumber.'<br>';
								...
								$strStatusType = _get_status_by_id('cloud_questions', $indicators["questionTable"], $this->session->userdata('mission_id'), 'fkidMission');
							...
							$arrDimensions = _get_record_by_fkid_md($indicators['piId'],'','cloud_questions');
								echo '<tr class="va">';
