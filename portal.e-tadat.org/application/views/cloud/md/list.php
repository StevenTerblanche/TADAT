...
//							echo '<th style="text-align:center !important; width:140px !important">' . strtoupper(lang('date').lang('_created')) . '</th>';
				...
				foreach ($get_record_all as $key => $value){
					...
					echo "<tr class='va'>";
						echo '<td>'._get_fields_by_id('PerformanceIndicators', 'id', $value['fkidPi'], $out = array('indicatorNumber','indicatorName'),': ','cloud_questions').'</td>';
//						echo '<td>'._get_fields_by_id('MeasurementDimensionTypes', 'id', $value['fkidDimensionType'], $out = array('typeName'),'','cloud_questions').'</td>';						
