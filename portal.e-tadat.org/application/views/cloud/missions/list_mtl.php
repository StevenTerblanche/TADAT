...
//							echo '<th>' . strtoupper(lang('revenue_authority')).'</th>';
				...
				foreach ($get_record_all_missions as $key => $value){
					...
					$strViewURI = base_url('missions/manage/mtl/?id=') . base64_encode($this->encrypt->encode($value['id']));
					$strViewURIProfile = base_url('missions/profile/mtl/?id=') . base64_encode($this->encrypt->encode($value['id']));					
					...
					echo "<tr class='va'>";
						...
						echo "<td class='va'>"._get_fields_by_id('MissionStatus', 'id' ,$value['fkidMissionStatus'], array('status', ' '))."</td>";
						...
						echo '<td class="va center">';
							echo '<div class="btn-group mr0 mb0">';
								...
								echo '<ul class="dropdown-menu dropdown-blue-bg">';
									echo '<li><a href="' . $strViewURI . '"><i class="fa fa-globe"></i>' . lang('manage').lang('_this').lang('_mission'). '</a></li>';
									echo '<li><a href="' . $strViewURIProfile. '"><i class="fa fa-globe"></i>' . lang('view').lang('_this').lang('_mission').lang('_profile'). '</a></li>';									
