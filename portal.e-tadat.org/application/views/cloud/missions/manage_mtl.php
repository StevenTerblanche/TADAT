		...
		if ($get_mission_details){?>
			...
			<div class="col-md-12 pl0 pr0 ml0 mr0 bt">
				<div class="col-md-12 pl0 pr0 text-muted">
					...
					<div class="col-md-12 pl0 pr0 text-muted">
					<?php 
echo '<p>The '.$get_mission_details->missionName. ', covering the assessment periods of ' . $get_mission_details->missionAssessmentPeriod . ', was created on behalf of the <a href="http://e-tadat.org/authorities/profile/?id='.base64_encode($this->encrypt->encode($get_mission_details->authorityId)).'" target="_blank">'.$get_mission_details->authorityName.'</a> Tax Administration, on '.date("j F Y", strtotime($get_mission_details->missionDateCreated)).', by '
.'<a href="http://e-tadat.org/users/profile/?id='.base64_encode($this->encrypt->encode($get_mission_details->secId)).'" target="_blank">'.$get_mission_details->secUserTitle.' '.$get_mission_details->secUserName.' '.$get_mission_details->secUserSurname.'</a> ('.$get_mission_details->secUserRole.').' 
.'</p>'; 
...
echo '<p>The primary contact person at the Tax Administration is <a href="http://e-tadat.org/users/profile/?id='.base64_encode($this->encrypt->encode($get_mission_details->racId)).'" target="_blank">'.$get_mission_details->racUserTitle.' '.$get_mission_details->racUserName.' '.$get_mission_details->racUserSurname.'</a> ('.$get_mission_details->racUserRole.').'; 
if($get_mission_details->raId){
	echo ' Secondary contact may also be made to <a href="http://e-tadat.org/users/profile/?id='.base64_encode($this->encrypt->encode($get_mission_details->raId)).'" target="_blank">'.$get_mission_details->raUserTitle.' '.$get_mission_details->raUserName.' '.$get_mission_details->raUserSurname.'</a> ('.$get_mission_details->raUserRole.').'; 
}
echo '</p>';							
/*
