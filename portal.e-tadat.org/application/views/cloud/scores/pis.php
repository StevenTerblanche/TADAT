<?php defined('BASEPATH') OR exit('No direct script access allowed'); 
			if ($get_record_all_pis){
				...
				foreach ($get_record_all_pis as $key => $value){
					$arrCount = _get_record_count_by_status($value['questionTable'], $field = 'fkidMission', $this->session->userdata('mission_id'), 'status', 'cloud_questions');
					$arrScores = _get_fields_by_id_array_clean('cloud_questions',$value['questionTable'], $field = 'fkidMission', $this->session->userdata('mission_id'), $out = array('score_symbol_overall'),100);
...
//print_r($arrScores);
					...
					echo "<tr class='va'>";
						...
						echo '<td>POA '._get_fields_by_id('Poa', 'id', $value['fkidPoa'], $out = array('poa_number','poa'),': ','cloud_questions').'</td>';						
