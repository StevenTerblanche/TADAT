...
//print_r($get_record_all_poas);
			if ($get_record_all_poas){
				...
				foreach ($get_record_all_poas as $key => $value){
					$arrIndicatorTables = _get_fields_by_id_array_clean('cloud_questions','PerformanceIndicators', 'fkidPoa', $value['id'], $out = array('questionTable'),10);	
					$strStatusCount = '';
					$strTblCount = '';
					$strScoreCount = 0;
					foreach ($arrIndicatorTables as $indicator => $indicators){
						$strTblCount++;
						$arrCount = _get_record_count_by_status($indicators['questionTable'], $field = 'fkidMission', $this->session->userdata('mission_id'), 'status', 'cloud_questions');
						if($arrCount){
