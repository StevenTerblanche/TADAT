<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	function _get_poa_intro_questions(){
		get_instance()->cloud_questions->select('Poa.id AS poaId, Poa.poa_number, Poa.poa AS poaName, Poa.description AS poaDescription, PerformanceIndicators.indicatorNumber, PerformanceIndicators.indicatorName, PerformanceIndicators.fkidPoa');
		get_instance()->cloud_questions->from('Poa');
		get_instance()->cloud_questions->join('PerformanceIndicators', 'PerformanceIndicators.fkidPoa = Poa.id');
	...
	function _get_record_all_scoring(){
		get_instance()->cloud_questions->order_by("MeasurementDimensionsScoring.fkidMd", "ASC");		
	...
	function _get_ra_pmq_questions(){
		get_instance()->cloud_questions->order_by("QuestionsPmq.sort_order", "ASC");
		get_instance()->cloud_questions->select('QuestionsPmq.*,QuestionsPmq.status as QuestionStatus, QuestionsPmq.id as QuestionId, QuestionTypes.*');
		get_instance()->cloud_questions->from('QuestionsPmq');
		get_instance()->cloud_questions->join('QuestionTypes', 'QuestionsPmq.fkidQuestionType = QuestionTypes.id');
		get_instance()->cloud_questions->where('QuestionsPmq.status', 1);
		$query = get_instance()->cloud_questions->get();
        if ($query->num_rows()){return $query->result_array();}
        return false;
	}
	...
	function _get_record_by_all_map_missions($CreatedUser = null, $UserType = null){
		get_instance()->db_cloud_main->select('Missions.*, Missions.id AS MissionID, RevenueAuthorities.*, Countries.country, Countries.lat, Countries.lng, MissionStatus.status, Users.fkidTitle, Users.name, Users.surname, Users.fkidUserRole');
		get_instance()->db_cloud_main->from('Missions');
		get_instance()->db_cloud_main->join('MissionStatus', 'Missions.fkidMissionStatus = MissionStatus.id');
		get_instance()->db_cloud_main->join('RevenueAuthorities', 'RevenueAuthorities.id = Missions.fkidRevenueAuthority');
		get_instance()->db_cloud_main->join('Users', 'RevenueAuthorities.fkidCounterpart = Users.id');
		get_instance()->db_cloud_main->join('Countries', 'RevenueAuthorities.fkidCountry = Countries.id');		
		if(!is_null($CreatedUser)){
			if(is_null($UserType)){
				get_instance()->db_cloud_main->where('Missions.fkidUserCreated', $CreatedUser);			
			}else{
				get_instance()->db_cloud_main->where('Missions.fkidTeamLeader', $CreatedUser);	
			}
		}
...
//		echo get_instance()->db->last_query();
	...
	function _get_record_all_indicators_by_poa(){
		get_instance()->cloud_questions = get_instance()->load->database('cloud_questions', TRUE); 
		get_instance()->cloud_questions->select('Poa.poa_number, Poa.poa, PerformanceIndicators.indicatorNumber, PerformanceIndicators.indicatorName');
		get_instance()->cloud_questions->from('Poa');
		get_instance()->cloud_questions->join('PerformanceIndicators', 'Poa.id = PerformanceIndicators.fkidPoa');
		$query = get_instance()->cloud_questions->get();
        if ($query->num_rows()){return $query->result_array();}
        return false;
	}
    ...
    function _get_record_all_logins($checkStatus = null, $checkTester = null){
		get_instance()->db->order_by("UsersLogins.date", "DESC");		
		get_instance()->db->select('UsersLogins.date, UsersLogins.fkidUser, Users.fkidTitle, Users.name, Users.surname, UserRoles.role');
		get_instance()->db->from('UsersLogins');
		get_instance()->db->join('Users', 'UsersLogins.fkidUser = Users.id');
		get_instance()->db->join('UserRoles', 'Users.fkidUserRole = UserRoles.id');
		if (!is_null($checkTester)) get_instance()->db->where_in('UsersLogins.tester', $checkTester);
		$query = get_instance()->db->get();
        if ($query->num_rows()){return $query->result_array();}
        return false;
    }
    ...
    function _get_record_all_rev_auth($checkStatus = null, $checkUser = null, $db = null){
		$db =& get_instance()->$db;
    ...
    function _get_record_all_missions($checkStatus = null, $checkUser = null, $checkRole = ''){
		get_instance()->db_cloud_main->from('Missions');
        if (!is_null($checkStatus)) get_instance()->db_cloud_main->where('status', 1);
        if (!is_null($checkUser)){
			if($checkRole === ''){
				 get_instance()->db_cloud_main->where('fkidUserCreated', $checkUser);	
			}else{
				get_instance()->db_cloud_main->where('fkidTeamLeader', $checkUser);
			}
		}
