...
//		$db->from('RightsAssignmentsIndicators');	
    ...
    function _get_mission_by_assigned_id($id = null){
		...
		$db->select('
			...
			');
		$db->from('Missions');	
		$db->join('AssessmentPeriod', 'AssessmentPeriod.id = Missions.fkidAssessmentPeriod','left');
		$db->join('RevenueAuthorities', 'RevenueAuthorities.id = Missions.fkidRevenueAuthority','left');			
		$db->join('Regions', 'Regions.id = RevenueAuthorities.fkidRegion','left');
		$db->join('Countries', 'Countries.id = RevenueAuthorities.fkidCountry','left');	
		$db->join('FederalStates', 'FederalStates.id = RevenueAuthorities.fkidState','left');		
		$db->join('Users AS raUsers', 'raUsers.id = RevenueAuthorities.fkidUser','left');
		$db->join('Users AS racUsers', 'racUsers.id = RevenueAuthorities.fkidCounterpart','left');
		$db->join('Users AS mtUsers', 'mtUsers.id = Missions.fkidTeamLeader','left');
		$db->join('Users AS secUsers', 'secUsers.id = Missions.fkidUserCreated','left');
		$db->join('Titles AS raTitles', 'raTitles.id = raUsers.fkidTitle','left');
		$db->join('Titles AS racTitles', 'racTitles.id = racUsers.fkidTitle','left');	
		$db->join('Titles AS mtTitles', 'mtTitles.id = mtUsers.fkidTitle','left');
		$db->join('Titles AS secTitles', 'secTitles.id = secUsers.fkidTitle','left');			

		$db->join('MissionStatus', 'MissionStatus.id = Missions.fkidMissionStatus','left');

		$db->join('UserRoles AS racUserRoles', 'racUserRoles.id = racUsers.fkidUserRole','left');	
		$db->join('UserRoles AS raUserRoles', 'raUserRoles.id = raUsers.fkidUserRole','left');	
		$db->join('UserRoles AS mtUserRoles', 'mtUserRoles.id = mtUsers.fkidUserRole','left');
		$db->join('UserRoles AS secUserRoles', 'secUserRoles.id = secUsers.fkidUserRole','left');			
        $db->where('Missions.id', $id);
		$query = $db->get();
//		echo $db->last_query();								
...
//echo $sql;
	...
	function _get_status_by_id_all($db = null, $tbl = null, $mission_id = null, $poa_id = null){
		...
		get_instance()->cloud_questions = get_instance()->load->database('cloud_questions', TRUE); 
		get_instance()->cloud_questions->select("PerformanceIndicators.questionTable AS piTable");
		get_instance()->cloud_questions->from($tbl);
        get_instance()->cloud_questions->where('PerformanceIndicators.fkidPoa', $poa_id);
        ...
        if ($query->num_rows()){
			...
			foreach($query->result_array() as $row => $value){
				get_instance()->cloud_questions = get_instance()->load->database('cloud_questions', TRUE); 
				get_instance()->cloud_questions->select($value['piTable'].".status");
		        get_instance()->cloud_questions->where($value['piTable'].'.fkidMission', $mission_id);
				get_instance()->cloud_questions->from($value['piTable']);
				$cnt++;
				$query_b = get_instance()->cloud_questions->get();
        		if ($query_b->num_rows()){
	...
	function _get_score_by_id_all($db = null, $tbl = null, $field = null, $id = null){
		...
		$db_selector = '';
		if($db == null){$db_selector = 'db';}else{$db_selector = $db;}
		get_instance()->cloud_questions->select("PerformanceIndicators.id AS piId, PerformanceIndicators.indicatorNumber,PerformanceIndicators.indicatorName, PerformanceIndicators.questionTable, PerformanceIndicators.assessment_text, Poa.id as poaId, Poa.poa_number, Poa.poa, Poa.assessment_text AS p_assessment_text");
		get_instance()->cloud_questions->from('PerformanceIndicators');
		get_instance()->cloud_questions->join('Poa', 'PerformanceIndicators.fkidPoa = Poa.id', 'left');
        get_instance()->cloud_questions->where(array('PerformanceIndicators.id' =>$id));
		$query = get_instance()->cloud_questions->get();
        if ($query->num_rows()){return $query->result_array();}
        return false;
}
	...
	function _get_record_by_id_score_md($id = null, $status = null, $db = null){
		...
		get_instance()->cloud_questions->join('ScoringMethods', 'MeasurementDimensions.fkidScoringMethod = ScoringMethods.id', 'right outer');				
	...
	function _get_record_by_id_score_table($id = null, $db = null, $tbl = null, $fields = array()){
        if (!$id) return false;
		$db_selector = '';
		if($db == null){$db_selector = 'db';}else{get_instance()->cloud_questions = get_instance()->load->database('cloud_questions', TRUE); $db_selector = $db;}
	...
	function _get_super_object_by_id($rid = null, $mid = null){
		...
		');
		$db->from('RevenueAuthorities');
		$db->join('Regions AS ra_regions', 'RevenueAuthorities.fkidRegion = ra_regions.id');
		$db->join('Countries AS ra_countries', 'RevenueAuthorities.fkidCountry = ra_countries.id');
		$db->join('FederalStates AS ra_state', 'RevenueAuthorities.fkidState = ra_state.id', 'left');
		$db->join('Users AS ram_user', 'ram_user.id = RevenueAuthorities.fkidUser');
		$db->join('Languages AS ram_language', 'ram_user.fkidLanguage = ram_language.id');
		$db->join('Regions AS ram_regions', 'ram_user.fkidRegion = ram_regions.id');
		$db->join('Countries AS ram_countries', 'ram_user.fkidCountry = ram_countries.id');
		$db->join('Missions', 'Missions.fkidRevenueAuthority = RevenueAuthorities.id');
		$db->join('AssessmentPeriod', 'AssessmentPeriod.id = Missions.fkidAssessmentPeriod');
		$db->join('MissionStatus', 'MissionStatus.id = Missions.fkidMissionStatus');
		$db->join('Users AS mtl_user', 'mtl_user.id = Missions.fkidTeamLeader');
		$db->join('Languages AS mtl_language', 'mtl_user.fkidLanguage = mtl_language.id');
		$db->join('Regions AS mtl_regions', 'mtl_user.fkidRegion = mtl_regions.id');
		$db->join('Countries AS mtl_countries', 'mtl_user.fkidCountry = mtl_countries.id');				

        $db->where(array('RevenueAuthorities.id'=>$rid));
		$query = $db->get();
	...
	function _get_poa_object_by_id($id = null){
       if (!$id) return false;
		get_instance()->cloud_questions->select("Poa.id, Poa.poa_number, Poa.poa, Poa.description, Poa.desiredOutcome, Poa.background, Poa.indicators");

		get_instance()->cloud_questions->from('Poa');
//		get_instance()->cloud_questions->join('PerformanceIndicators', 'PerformanceIndicators.fkidPoa = Poa.id', 'left outer');
//		get_instance()->cloud_questions->join('MeasurementDimensions', 'MeasurementDimensions.fkidPi = PerformanceIndicators.id');		
//		get_instance()->cloud_questions->join('MeasurementDimensionTypes', 'MeasurementDimensions.fkidDimensionType = MeasurementDimensionTypes.id');		
        get_instance()->cloud_questions->where(array('Poa.id'=>$id));	
		$query = get_instance()->cloud_questions->get();
        if ($query->num_rows()){return $query->row_array();}
        return false;	
	}
	...
	function _get_record_by_id_all_uploaded($missionId = null, $indicatorId = null, $dimensionId = null, $uploadType = null, $table = null, $db = null){
		...
		$db_selector = '';
		if($db == null){$db_selector = 'db';}else{$db_selector = $db;}
		get_instance()->$db_selector->select('documentTitle, documentDescription, documentSize, documentType, documentPath, thumbPath, dateCreated');
		get_instance()->$db_selector->from($table);
        if ($uploadType == 1){
	        if ($dimensionId  == null){
	...
	function _get_current_missions_by_id($id = null, $role = null){
        if (!$id) return false;
		$db->select('RevenueAuthorities.*,UserRoles.*, Missions.*, AssessmentPeriod.period, Countries.country,
		...
		$db->from('RevenueAuthorities');
		$db->join('Missions', 'Missions.fkidRevenueAuthority = RevenueAuthorities.id');
		$db->join('Countries', 'RevenueAuthorities.fkidCountry = Countries.id');
		$db->join('AssessmentPeriod', 'AssessmentPeriod.id = Missions.fkidAssessmentPeriod');
		$db->join('Users', 'Users.id = Missions.fkidTeamLeader');
		$db->join('UserRoles', 'UserRoles.id = Users.fkidUserRole');

		if($role && $role == 7){
	        $db->where(array('RevenueAuthorities.fkidCounterpart'=>$id, 'Missions.fkidMissionStatus'=>3));			
		}else{
	        $db->where(array('RevenueAuthorities.fkidUser'=>$id, 'Missions.fkidMissionStatus'=>3));						
		}
...
/*
	function _get_user_assignments_by_id($id = null){
        if (!$id) return false;
		$db->select('fkidAuthority, fkidMission, pmq, poa_1, poa_2, poa_3, poa_4, poa_5, poa_6, poa_7, poa_8, poa_9');
		$db->from('RightsAssignments');
		$db->where('RightsAssignments.fkidUser',$id, 1);		
		$query = $db->get();
        if ($query->num_rows()){return $query->result_array();}
        return false;
	}
*/	
	function _get_user_assignments_by_id($id = null){
        if (!$id) return false;
		$db->select('fkidAuthority, fkidMission, `P1-1`, `P1-2`, `P2-3`, `P2-4`, `P2-5`, `P2-6`, `P3-7`, `P3-8`, `P3-9`, `P4-10`, `P4-11`, `P5-12`, `P5-13`, `P5-14`, `P5-15`, `P6-16`, `P6-17`, `P6-18`, `P7-19`, `P7-20`, `P7-21`, `P8-22`, `P8-23`, `P8-24`, `P9-25`, `P9-26`, `P9-27`, `P9-28`');
		$db->from('RightsAssignmentsIndicators');
		$db->where('RightsAssignmentsIndicators.fkidUser',$id, 1);		
		$query = $db->get();
        if ($query->num_rows()){return $query->result_array();}
        return false;
	}	
	...
	function _get_record_by_id_indicators($id = null, $status = null, $db = null){
		...
		$db_selector = '';
		if($db == null){$db_selector = 'db';}else{$db_selector = $db;}
		get_instance()->cloud_questions->select("Poa.id as poaId, Poa.poa_number, Poa.poa, PerformanceIndicators.id AS piId, PerformanceIndicators.indicatorNumber,PerformanceIndicators.indicatorName");
		get_instance()->cloud_questions->from('PerformanceIndicators');
		get_instance()->cloud_questions->join('Poa', 'PerformanceIndicators.fkidPoa = Poa.id', 'right outer');
        get_instance()->cloud_questions->where('PerformanceIndicators.id',$id, 1);
		$query = get_instance()->cloud_questions->get();
        if ($query->num_rows()){return $query->result_array();}
        return false;
}
	...
	function _get_poa_by_id($id = null, $status = null, $db = null){
		...
		get_instance()->cloud_questions->select("Poa.id, Poa.poa_number, Poa.poa, Poa.description, Poa.desiredOutcome, Poa.background, Poa.indicators");
		get_instance()->cloud_questions->from('Poa');
        get_instance()->cloud_questions->where('Poa.id',$id, 1);
		$query = get_instance()->cloud_questions->get();
        if ($query->num_rows()){return $query->result_array();}
        return false;
}
	...
	function _get_indicator_by_indicator_id($id = null, $status = null){
        if (!$id) return false;
		get_instance()->cloud_questions = get_instance()->load->database('cloud_questions', TRUE); 
		get_instance()->cloud_questions->select('*,PerformanceIndicators.id as piId, ScoringMethods.methodName, ScoringMethods.methodType');
		get_instance()->cloud_questions->from('PerformanceIndicators');
		get_instance()->cloud_questions->join('ScoringMethods', 'PerformanceIndicators.fkidScoringMethod = ScoringMethods.id','left');
        get_instance()->cloud_questions->where('PerformanceIndicators.id',$id);
		$query = get_instance()->cloud_questions->get();
        if ($query->num_rows()){return $query->result_array();}
        return false;
	}
	...
	function _get_record_by_id_assignment($id = null, $status = null){
		...
		$db->select('*');
		$db->from('RightsAssignmentsIndicators');
        $db->where('RightsAssignmentsIndicators.id',$id, 1);
		$query = $db->get();
        if ($query->num_rows()){return $query->row();}
        return false;
	}
	...
	function _get_record_by_id_glossary($id = null, $status = null){
        if (!$id) return false;
		$db->order_by("Glossary.term", "ASC");	
		$db->select('*');
		$db->from('Glossary');
        $db->where('Glossary.id',$id, 1);
		$query = $db->get();
        if ($query->num_rows()){return $query->row();}
        return false;
	}
	...
	function _get_record_by_id_score($id = null, $status = null){
		...
		get_instance()->cloud_questions->from('MeasurementDimensionsScoring');
        get_instance()->cloud_questions->where('MeasurementDimensionsScoring.id',$id, 1);
		$query = get_instance()->cloud_questions->get();
        if ($query->num_rows()){return $query->row();}
        return false;
	}
	...
	function _get_fields_by_id_poa_ev($id = null, $status = null){
        if (!$id) return false;
		get_instance()->cloud_questions->select("CONCAT(PerformanceIndicators.fkidPoa,'- Question ', PerformanceIndicators.indicatorNumber,' (', MeasurementDimensionTypes.typeName, '): ', MeasurementDimensions.dimensionName) AS 'poa'");
		...
		get_instance()->cloud_questions->join('MeasurementDimensionTypes', 'MeasurementDimensions.fkidDimensionType = MeasurementDimensionTypes.id');		
        get_instance()->cloud_questions->where('MeasurementDimensions.id',$id, 1);
		$query = get_instance()->cloud_questions->get();
        if ($query->num_rows()){return $query->result_array();}
        return false;
	}	
	...
	function _get_record_by_id_users($id = null, $status = null){
        if (!$id) return false;
		get_instance()->db_portal->select('Users.*');
		get_instance()->db_portal->from('Users');
//		get_instance()->db_portal->join('UserRoles', 'Users.fkidUserRole = UserRoles.id');

        get_instance()->db_portal->where('Users.id',$id);
        if($status) get_instance()->db_portal->where('Users.status', $status, 1);
	...
	function _get_temp_record_by_id_users($id = null){
        if (!$id) return false;
		get_instance()->db_portal->select('Users.*');
		get_instance()->db_portal->from('Users');
        get_instance()->db_portal->where('Users.id',$id, 1);
		$query = get_instance()->db_portal->get();
        if ($query->num_rows()){
			return $query->row();
			}
	...
	function _get_record_by_id_missions($id = null){
        if (!$id) return false;
		$db->select('Missions.*, Users.fkidTitle as mTitle, Users.name as mName, Users.surname as mSurname, UserRoles.role as mRole');
		$db->from('Missions');
		$db->join('Users', 'Missions.fkidUserCreated = Users.id');
		$db->join('UserRoles', 'Users.fkidUserRole = UserRoles.id');
		$db->join('RevenueAuthorities', 'RevenueAuthorities.id = Missions.fkidRevenueAuthority');
        $db->where('Missions.id',$id, 1);
		$query = $db->get();
        if ($query->num_rows()){return $query->row();}
        return false;
}
	...
	function _get_record_by_id_missions_where_assigned($userid = null){
		...
		$db->select('*');
		$db->from('Missions');
        $db->where('Missions.fkidTeamLeader',$userid);
		$query = $db->get();
        if ($query->num_rows()){return $query->result_array();}
        return false;
}
	...
	function _get_users_by_id_missions($id = null){
        if (!$id) return false;
		$db->select('Missions.fkidTeamLeader as id_mtl, RevenueAuthorities.fkidCounterpart as id_counterpart, RevenueAuthorities.fkidUser as id_admin');
		$db->from('Missions');
		$db->join('RevenueAuthorities', 'RevenueAuthorities.id = Missions.fkidRevenueAuthority');
        $db->where('Missions.id',$id, 1);
		$query = $db->get();
	...
	function _get_ra_users_by_id_missions($id = null){
        if (!$id) return false;
		$db->select('RevenueAuthorities.fkidCounterpart as id_counterpart, RevenueAuthorities.fkidUser as id_admin');
		$db->from('Missions');
		$db->join('RevenueAuthorities', 'RevenueAuthorities.id = Missions.fkidRevenueAuthority');
        $db->where('Missions.id',$id, 1);
		$query = $db->get();
	...
	function _get_user_assignments_by_id_missions($assignment_id = null, $mission_id = null, $authority_id = null, $user_id = null){
		...
		$db->select('Missions.mission as mission_name, Missions.dateStart as startDate, Missions.dateEnd as endDate, RevenueAuthorities.authority as authority_name, RevenueAuthorities.fkidRegion as authority_region, RevenueAuthorities.fkidCountry as authority_country, RevenueAuthorities.city as authority_city');
		$db->from('RightsAssignmentsIndicators');
		$db->join('RevenueAuthorities', 'RevenueAuthorities.id = RightsAssignmentsIndicators.fkidAuthority');
		$db->join('Missions', 'Missions.id = RightsAssignmentsIndicators.fkidMission');
        $db->where('RightsAssignmentsIndicators.id',$assignment_id);
		$query = $db->get();
        if ($query->num_rows()){return $query->row();}
        return false;
}
    ...
    function _get_record_by_id_poa_name($id = null){
		get_instance()->cloud_questions = get_instance()->load->database('cloud_questions', TRUE);
		get_instance()->cloud_questions->select('poa');
		get_instance()->cloud_questions->from('Poa');
        get_instance()->cloud_questions->where('Poa.id',$id, 1);
		$query = get_instance()->cloud_questions->get();
        if ($query->num_rows()){return $query->row();}
	...
	# REVENUE AUTHORITIES BY ID
    function _get_record_by_id_rev_auth($id = null, $db = null){
        if (!$id || !$db) return false;
		$db =& get_instance()->$db;
		$db->select('RevenueAuthorities.*, Users.fkidTitle, Users.name, Users.surname, UserRoles.role, Countries.country, Regions.region');
		$db->from('RevenueAuthorities');
		$db->join('Users', 'RevenueAuthorities.fkidUserCreated = Users.id');
		$db->join('UserRoles', 'Users.fkidUserRole = UserRoles.id');
		$db->join('Countries', 'RevenueAuthorities.fkidCountry = Countries.id');
		$db->join('Regions', 'RevenueAuthorities.fkidRegion = Regions.id');
        $db->where('RevenueAuthorities.id',$id, 1);
        $db->where('Users.status', 1, 1);
		$query = $db->get();
        if ($query->num_rows()){return $query->row();}
        return false;
    }
	...
	function _get_record_by_id_rev_auth_ram($id = null, $db = null){
        if (!$id || !$db) return false;
		$db =& get_instance()->$db;
		$db_b =& get_instance()->db_cloud_main;		
		$db->select('Users.id as raId, Users.fkidTitle as raTitle, Users.name as raName, Users.surname as raSurname, Users.description as raDescription, 
		...
		$db_b->from('RevenueAuthorities');
		$db->join('Users', 'RevenueAuthorities.fkidUser = Users.id');
		$db->join('Languages', 'Users.fkidLanguage = Languages.id');
		$db->join('Countries', 'Users.fkidCountry = Countries.id');
		$db->join('Regions', 'Users.fkidRegion = Regions.id');
		$db->join('UserRoles', 'Users.fkidUserRole = UserRoles.id');	
        $db_b->where(array('RevenueAuthorities.id'=>$id), 1);
		$query = $db_b->get();
        if ($query->num_rows()){return $query->row();}
        return false;	
	}
	...
	# REVENUE AUTHORITY MANAGER BY ID
	function _get_record_by_id_rev_auth_counterpart($id = null, $db = null){
        if (!$id || !$db) return false;
		$db =& get_instance()->$db;
		$db_b =& get_instance()->db_cloud_main;		
		$db->select('Users.id as raId, Users.fkidTitle as raTitle, Users.name as raName, Users.surname as raSurname, Users.description as raDescription, 
									...
									UserRoles.role  as raRole, Languages.language  as raLanguage');
		$db_b->from('RevenueAuthorities');
		$db->join('Users', 'RevenueAuthorities.fkidCounterpart = Users.id');
		$db->join('Languages', 'Users.fkidLanguage = Languages.id');
		$db->join('Countries', 'Users.fkidCountry = Countries.id');
		$db->join('Regions', 'Users.fkidRegion = Regions.id');
		$db->join('UserRoles', 'Users.fkidUserRole = UserRoles.id');	
	...
	function _get_record_by_id_team_leader($id = null){
        if (!$id) return false;
		$db->select('Users.id as tlId, Users.fkidTitle as tlTitle, Users.name as tlName, Users.surname as tlSurname, Users.description as tlDescription, 
		...
		$db->from('Missions');
		$db->join('Users', 'Missions.fkidTeamLeader = Users.id');
		$db->join('Languages', 'Users.fkidLanguage = Languages.id');
		$db->join('Countries', 'Users.fkidCountry = Countries.id');
		$db->join('Regions', 'Users.fkidRegion = Regions.id');
		$db->join('UserRoles', 'Users.fkidUserRole = UserRoles.id');	
        $db->where(array('Users.id'=>$id), 1);
		$query = $db->get();
        if ($query->num_rows()){return $query->row();}
        return false;	
	}	
	...
	function _get_record_by_id_templates($id = null){
        if (!$id) return false;
		$db->select('Templates.*, TemplateTypes.nameTemplateType as ttName, TemplateTypes.uinameTemplateType as ttUIName, TemplateTypes.typeDueTime as ttDue, TemplateTypes.typePages as ttPages');
		$db->from('Templates');
		$db->join('TemplateTypes', 'Templates.fkidTemplateType = TemplateTypes.idTemplateTypes');
        $db->where('Templates.idTemplates',$id, 1);
		$query = $db->get();
        if ($query->num_rows()){return $query->row();}
        return false;
	}
	...
	function _get_record_by_id_fields($id = null){
        if(!$id) return false;
		$query = $db->get_where('TemplateFields', array('fkidTemplates' => $id));
        if($query->num_rows()){
